...
echo -e "$GREEN We can through different options. Let's get started! (to install a node, go through 1st step and then 2nd one)"
echo -e "$NORMAL-------------------------------------------------------------------"
...
echo "4) NODE STATUS$NORMAL"
echo "-------------------------------------------------------------------"
echo -e "$YELLOW What do you want to do? $NORMAL"
echo "-------------------------------------------------------------------"
...
if [ "$OPTION" == "1" ]; then
	...
	while true; do 
		...
		$BEHIND=$(echo $RPCBLOCK - $YOURBLOCK)
